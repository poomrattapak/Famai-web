...
| 29 | `29_writeback_columns` | v1.49 (เขียนกลับครบตาราง): `registration.stage_log` jsonb (ประวัติเดินขั้น — careCreate อ่านวันได้ป้ายตาม §9i) · `expense.category` text ตามบรีฟหมวดพิมพ์เสรี + `category_id` เลิกบังคับ — apply แล้ว ตรวจชนิดคอลัมน์จริงครบ (`follow_up_task.done_by` เป็น uuid ⇒ ฝั่งแอป guard ด้วย `isUuid(ME.id)`) |
| 30 | `30_unit_v_money_mask` | v1.50 (ตัดเงินที่ต้นทาง): วิว `unit_v` (`security_invoker=true` — RLS สาขาเดิมยังคุม) แบนรุ่น/สาขาเข้าวิว + `cost`/`cost_vat` เป็น `case when app_can_money() …` · helper `app_can_money()` อ่าน role + `app_setting('perms')` กติกาเดียวกับ `permR('data:money')` ฝั่งแอป · revoke anon / grant authenticated — **รายชื่อคอลัมน์ของวิวคือเส้นแบ่งความปลอดภัย ห้าม `select *`** · trigger `sale_fill_cost` เติมทุนจริง+กำไร ณ วันขายเมื่อแอปส่ง null (คนไม่มีสิทธิ์เงินขายรถ — §9i) — **ยิงพิสูจน์แล้ว**: สวม JWT sales1 → 29 แถว cost null หมด · acct → 50 แถว cost ครบ |
| 31 | `31_gen_token_grant` | v1.50 (พบตอนยิงพิสูจน์): default ของ `sale.public_token` เรียก `pub.gen_token()` แต่ `authenticated` ไม่มีสิทธิ์ execute — insert ที่ไม่ส่ง token เองพัง 42501 ทั้งแถว (แอปรอดเพราะ `saveSale` ส่งเอง แต่ invariant "ทุกทางได้ token" ไม่เคยจริง) · grant execute แล้วยิงพิสูจน์: insert ไม่ส่ง token → token ออกจาก default สำเร็จ |
