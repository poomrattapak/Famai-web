...
| 27 | `27_wholesale_void` | ยกเลิกบิลขายส่ง (v1.45): `wholesale_sale` เพิ่ม `voided_reason` · `voided_by` · `dest_branch_id` (สาขาปลายทางของบิลในเครือ — เดิมไม่เคยเก็บ) · index บางส่วน `where voided_at is null` · ปิดรู `ws_sale_upd` ที่มีแต่ `using` ไม่มี `with check` (แก้บิลแล้วย้าย `branch_id` ออกนอกขอบเขตตัวเองได้) — **`voided_at` มีอยู่แล้วตั้งแต่ migration 17 แต่ไม่เคยมีโค้ดเขียน** และตารางไม่มี DELETE policy ⇒ soft void เป็นทางเดียว |
